...
typedef thrust::complex<double> cmplx;

namespace gEPIC{
    __host__ __device__ constexpr double NaN(){return nan("");}
    __host__ __device__ double erfcx(double);
    __host__ __device__ double w_im(double);
}
...
namespace gEPIC{
    ...
    __host__ __device__
    static inline double sinc(double x, double sinx) {return fabs(x) < 1e-4 ? 1 - (0.1666666666666666666667)*x*x : sinx / x;}
    ...
    cmplx wofz(const cmplx &z){
        ...
        static const double expa2n2[] = {
                7.64405281671221563e-01,3.41424527166548425e-01,8.91072646929412548e-02,1.35887299055460086e-02,1.21085455253437481e-03,6.30452613933449404e-05,
                1.91805156577114683e-06,3.40969447714832381e-08,3.54175089099469393e-10,2.14965079583260682e-12,7.62368911833724354e-15,1.57982797110681093e-17,
                1.91294189103582677e-20,1.35344656764205340e-23,5.59535712428588720e-27,1.35164257972401769e-30,1.90784582843501167e-34,1.57351920291442930e-38,
                7.58312432328032845e-43,2.13536275438697082e-47,3.51352063787195769e-52,3.37800830266396920e-57,1.89769439468301000e-62,6.22929926072668851e-68,
                1.19481172006938722e-73,1.33908181133005953e-79,8.76924303483223939e-86,3.35555576166254986e-92,7.50264110688173024e-99,9.80192200745410268e-106,
                7.48265412822268959e-113,3.33770122566809425e-120,8.69934598159861140e-128,1.32486951484088852e-135,1.17898144201315253e-143,6.13039120236180012e-152,
                1.86258785950822098e-160,3.30668408201432783e-169,3.43017280887946235e-178,2.07915397775808219e-187,7.36384545323984966e-197,1.52394760394085741e-206,
                1.84281935046532100e-216,1.30209553802992923e-226,5.37588903521080531e-237,1.29689584599763145e-247,1.82813078022866562e-258,1.50576355348684241e-269,
                7.24692320799294194e-281,2.03797051314726829e-292,3.34880215927873807e-304,0.0 // underflow (also prevents reads past array end, below)
        };
        if(z.real()==0.0)
            return cmplx(erfcx(z.imag()),z.real());
        else if(z.imag()==0.0)
            return cmplx(exp(-sqr(z.real())),w_im(z.real()));
        ...
        if (ya > 7 || (x > 6  // continued fraction is faster
            ...
            if (y < 0) {
                ...
                return 2.0*exp(cmplx((ya-xs)*(xs+ya), 2*xs*y)) - ret;
            }       
            else
                return ret;
        }
           ...
           below.  Therefore, we use x < 10 here. */
            ...
            if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
                ...
                for (int n = 1; 1; ++n) {
                    ...
                    if (coef * prod2ax < DBL_EPSILON * sum3) break;
                }
            }
                ...
                for (int n = 1; 1; ++n) {
                    ...
                    if ((coef * prod2ax) * (a*n) < DBL_EPSILON * sum5) break;
                }
            }
            //} not copy relerr != DBL_EPSILON part
            ...
            if (y > 5) { // imaginary terms cancel
                ...
                double sin2xy,cos2xy;sincos(2*xs*y,&sin2xy,&cos2xy);
            ...
            for (dn = 1; n0 - dn > 0; ++dn) { // loop over n0-dn and n0+dn terms
                ...
                sum3 += tp + tm; 
                sum5 += a * (np * tp + nm * tm); 
                if (a * (np * tp + nm * tm) < DBL_EPSILON * sum5) goto finish;
            ...
            while (1) { // loop over n0+dn terms only (since n0-dn <= 0)
                ...
                sum3 += tp;
                sum5 += a * np * tp;
                if (a * np * tp < DBL_EPSILON * sum5) goto finish;
            }
