...
int main(int argc, char* argv[]){
    ...
    double rot_ang=DBL0(Global,rotated_angle);
    double ax_ang=DBL0(Global,axis_angle);
    ...
    //LinearY
    linear_beta1=DBL0(CC2IP1,betay);
    linear_beta2=DBL1(CC2IP1,betay);
    linear_alpha1=DBL0(CC2IP1,alphay);
    linear_alpha2=DBL1(CC2IP1,alphay);
    linear_phase=DBL0(CC2IP1,dphasey);
    LinearY MY11(linear_beta2,linear_alpha2,linear_beta1,linear_alpha1,-linear_phase);
    LinearY MY21(linear_beta1,linear_alpha1,linear_beta2,linear_alpha2,linear_phase);

    linear_beta1=DBL0(IP2CC1,betay);
    linear_beta2=DBL1(IP2CC1,betay);
    linear_alpha1=DBL0(IP2CC1,alphay);
    linear_alpha2=DBL1(IP2CC1,alphay);
    linear_phase=DBL0(IP2CC1,dphasey);
    LinearY MY31(linear_beta1,linear_alpha1,linear_beta2,linear_alpha2,linear_phase);
    LinearY MY41(linear_beta2,linear_alpha2,linear_beta1,linear_alpha1,-linear_phase);

    linear_beta1=DBL0(CC2IP2,betay);
    linear_beta2=DBL1(CC2IP2,betay);
    linear_alpha1=DBL0(CC2IP2,alphay);
    linear_alpha2=DBL1(CC2IP2,alphay);
    linear_phase=DBL0(CC2IP2,dphasey);
    LinearY MY12(linear_beta2,linear_alpha2,linear_beta1,linear_alpha1,-linear_phase);
    LinearY MY22(linear_beta1,linear_alpha1,linear_beta2,linear_alpha2,linear_phase);

    linear_beta1=DBL0(IP2CC2,betay);
    linear_beta2=DBL1(IP2CC2,betay);
    linear_alpha1=DBL0(IP2CC2,alphay);
    linear_alpha2=DBL1(IP2CC2,alphay);
    linear_phase=DBL0(IP2CC2,dphasey);
    LinearY MY32(linear_beta1,linear_alpha1,linear_beta2,linear_alpha2,linear_phase);
    LinearY MY42(linear_beta2,linear_alpha2,linear_beta1,linear_alpha1,-linear_phase);

    //ThinCrabCavity
    auto set_hcc=[](const string &scope, ThinCrabCavity2D &tcc) -> void {
    ...
    double tcc_frequency,tcc_strengthX,tcc_strengthY,tcc_phase;

    tcc_frequency=DBL0(ThinCrabCavity_before_IP1,frequency);
    tcc_strengthX=DBL0(ThinCrabCavity_before_IP1,strength);
    tcc_strengthY=DBL1(ThinCrabCavity_before_IP1,strength);
    tcc_phase=DBL0(ThinCrabCavity_before_IP1,phase);
    ThinCrabCavity2D tccb1(tcc_strengthX,tcc_strengthY,tcc_frequency,tcc_phase);
    set_hcc("ThinCrabCavity_before_IP1",tccb1);

    tcc_frequency=DBL0(ThinCrabCavity_after_IP1,frequency);
    tcc_strengthX=DBL0(ThinCrabCavity_after_IP1,strength);
    tcc_strengthY=DBL1(ThinCrabCavity_after_IP1,strength);
    tcc_phase=DBL0(ThinCrabCavity_after_IP1,phase);
    ThinCrabCavity2D tcca1(tcc_strengthX,tcc_strengthY,tcc_frequency,tcc_phase);
    set_hcc("ThinCrabCavity_after_IP1",tcca1);

    tcc_frequency=DBL0(ThinCrabCavity_before_IP2,frequency);
    tcc_strengthX=DBL0(ThinCrabCavity_before_IP2,strength);
    tcc_strengthY=DBL1(ThinCrabCavity_before_IP2,strength);
    tcc_phase=DBL0(ThinCrabCavity_before_IP2,phase);
    ThinCrabCavity2D tccb2(tcc_strengthX,tcc_strengthY,tcc_frequency,tcc_phase);
    set_hcc("ThinCrabCavity_before_IP2",tccb2);

    tcc_frequency=DBL0(ThinCrabCavity_after_IP2,frequency);
    tcc_strengthX=DBL0(ThinCrabCavity_after_IP2,strength);
    tcc_strengthY=DBL1(ThinCrabCavity_after_IP2,strength);
    tcc_phase=DBL0(ThinCrabCavity_after_IP2,phase);
    ThinCrabCavity2D tcca2(tcc_strengthX,tcc_strengthY,tcc_frequency,tcc_phase);
    set_hcc("ThinCrabCavity_after_IP2",tcca2);


    //Lorentz boost and reverse Lorentz boost
    auto tMs=rotate_matrix(angle,rot_ang,ax_ang);
    const auto &EX=std::get<0>(tMs);
    const auto &EP=std::get<1>(tMs);
    const auto &IX=std::get<2>(tMs);
    const auto &IP=std::get<3>(tMs);
    RotatedLorentzBoost lb(EX,EP);
    RotatedRevLorentzBoost rlb(EX.inverse(),EP.inverse());
    ...
    */
    ...
    while(current_turn<total_turns){
        for(int i=0;i<output_turns;++i){
            ...
            wb1.Pass(MX11,MY11,tccb1,MY21,MX21,lb);
            wb2.Pass(MX12,MY12,tccb2,MY22,MX22,lb);
            ...
            wb1.Pass(rlb,MX31,MY31,tcca1,MY41,MX41,ot1,rad1);//rad1.do_for(wb1);
            wb2.Pass(rlb,MX32,MY32,tcca2,MY42,MX42,ot2,rad2);//rad2.do_for(wb2);
