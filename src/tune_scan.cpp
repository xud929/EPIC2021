    ...
    while(current_nux<nux_end){
        ...
        while(current_nuy<nuy_end){
            ...
            for(unsigned i=0;i<output_times;++i){
                vector<double> temp;
                if(i)
                    temp=gg.track2(fit_step);
                else
                    temp=gg.track2(fit_start);
                ...
                for(unsigned j=0;j<2;++j){
                    double &um=beam_data[i][1+j*2], &us=beam_data[i][2+j*2];
                    us-=um*um;
                    us=us>0?sqrt(us):0;
                }
            ...
            if(rank==0){
                ...
                for(unsigned i=0;i<output_times;++i){
                    ...
                    sx_mean+=beam_data[i][2];
                    sy_mean+=beam_data[i][4];
                    x_sx_mean+=turns[i]*beam_data[i][2];
                    x_sy_mean+=turns[i]*beam_data[i][4];
                }
                x_mean/=output_times;
                ...
                const auto &vback=beam_data.back();
